...
public class NewtonData6 extends NewtonDataDriven {
	double c; 
...
//			System.out.println("f_new:"+f_new+",f:"+f+",delta:"+ alpha_line_search*out*gradient.scalarProduct(direction));
	...
	public NewtonData6(SecondOrderLoss loss, SampleSizeStrategy as, double c) {
		super(loss,as,c);
		this.c = c; 
		this.as = as; 
	...
	public void update_iterations(double time) {
		this.time+= time;
		if(initialSample == as.getSubsamplesi()){
			num_computed_gradients += as.getSubsamplesi();
		}
		else if(curss!=as.getSubsamplesi()){
			num_computed_gradients += as.getSubsamplesi();
		}
	    if(curss == loss.getDataSize()){
	    	num_computed_gradients += as.getSubsamplesi(); 
	    }
	}
	int curss; 
...
//			step_size = backtracking_line_search(delta); 
	...
	@Override
	public void setName() {
		this.name = "ExactNewton"; 
	}

	@Override
	public FirstOrderOpt clone_method() {
		NewtonData6 method = new NewtonData6((SecondOrderLoss) loss.clone_loss(),as,c); 
		method.w = clone_w(); 
