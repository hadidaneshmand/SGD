...
//		Logistic_Loss_efficient.buildHessians(Input.data, Input.d);
		...
		FirstOrderOpt[] methods_in = new FirstOrderOpt[6]; 
		...
		methods_in[5] = new Nesterov2(loss.clone_loss(),new GD(loss.clone_loss())); 
		for(int i=0;i<methods_in.length;i++){
			methods_in[i].setParam(initParam);
		}
		First_Order_Factory_efficient.methods_in = methods_in; 
		First_Order_Factory_efficient.experiment_with_iterations_complexity(1, loss.clone_loss(), 120, -1.0, Input.loss_test, Input.config.logDir+"_newton_time", Input.L, false,n);
	}
}